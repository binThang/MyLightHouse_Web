...
use stdClass;
class metoomodel Extends Appmodel
{
	...
	//리스트
	public function metooList($kind, $idx, $page = 1)
	{
		$offset=($page-1)*5;
		$tonysql = '';
		...
		switch($kind){
			case 3 :
				...
				// $orderby = ' WHERE ORDER BY wdate DESC';
				$where = '';
				break;
			case 2 :
		...
		$sql="SELECT
				metoo_idx, subject, content, board.color, hashtag, category.category, wdate, content_url,
				(
					SELECT count(*)
					FROM {$this->dbname}_favorite AS favorite
					WHERE favorite.board_idx = board.metoo_idx
					AND member_idx = $this->member_idx and kind = 4 and ifreply = 0) AS con,
				(SELECT count(*)
				 FROM {$this->dbname}_favorite AS favorite
				 WHERE board_idx = board.metoo_idx
				 and kind = 4 and ifreply = 0) AS allcount,
				(SELECT count(*)
				 FROM {$this->dbname}_reply
			 	 WHERE kind = 4 AND board_idx = board.metoo_idx) AS replycon
			FROM {$this->dbname}_metoo AS board
	...
	{
		$sql="SELECT metoo_idx, category.category, hashtag, subject, content, board.color, content_url, wdate,
			(SELECT member_type FROM {$this->dbname}_member WHERE member_idx = $this->member_idx) AS member_type
			FROM {$this->dbname}_metoo AS board
			JOIN {$this->dbname}_category AS category ON board.category = category.category_idx
			WHERE metoo_idx = $idx";
		$query=$this->db->prepare($sql);
	...
	public function favoriteReplyCon($board = null)
	{
		$sql="SELECT
				...
				(SELECT count(*)
				 ...
				 WHERE board_idx = $board and kind = 4 AND ifreply = 0) allcount,
				(SELECT count(*)
				 FROM {$this->dbname}_reply
				 WHERE board_idx = ? and kind = 4) replycon,
				(SELECT count(*)
				 FROM {$this->dbname}_singo
				 WHERE board_idx = ? and kind = 4 and ifreply = 0) singocon
			FROM {$this->dbname}_favorite
			WHERE board_idx = ? AND member_idx = ? and ifreply = 0 and kind = 4";
		$query=$this->db->prepare($sql);
	...
	public function insert_reply($board = null)
	{
		...
		$sqla="INSERT into {$this->dbname}_reply (member_idx, content, board_idx, kind, rdate, content_image) values (?,?,?,4,now(),?)";
	...
	public function list_reply($idx){
		$sql="SELECT
			...
			WHERE reply.board_idx = ? AND reply.kind = 4
	...
	public function singo(){
		...
		$query=$this->db->prepare($sql);
		$query->execute(array($_POST['board_idx'],$this->member_idx,4,$ifreply));
		$con = $query->fetch()->con;
		if($con){
			...
			$query=$this->db->prepare($sql);
			$query->execute(array($_POST['board_idx'],$this->member_idx,4,$ifreply,$text));
			if($query->rowCount()){
				return array('return'=>'true');
			}else{
				return array('return'=>'fail','text'=>'등록 실패',$sql,$array);
	...
	public function reload($idx)
	{
		$sql="SELECT
				...
				(SELECT count(*)
				 FROM {$this->dbname}_favorite
				 WHERE board_idx = $idx and kind = 4 AND ifreply = 0) allcount,
				(SELECT count(*)
				 FROM {$this->dbname}_reply
				 WHERE board_idx = ? and kind = 4) replycon,
				(SELECT count(*)
