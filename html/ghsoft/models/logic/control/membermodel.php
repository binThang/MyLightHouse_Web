...
use stdClass;
class membermodel Extends Controlmodel
{
  ...
  public function list($page)
  {
	...
	//   echo $_SESSION[$this->type->method]['type'];
	//   switch($_SESSION[$this->type->method]['type']){
	if($_POST['name']){
	    $_SESSION['name'] = $_POST['name'];
	}
	if($_SESSION['name']){
		$like = ' AND member_id LIKE "%'.$_SESSION['name'].'%"';
	}
	if($_POST['set']){
		unset($_SESSION['name']);
	}
