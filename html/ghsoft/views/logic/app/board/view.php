<section>
    ...
    <script type="text/javascript">
        $(function(){
            var my = "<?=$this->myboard->con?>";
            var board_idx = "<?=$this->view->board_idx?>";
            var category = "<?=$this->view->category?>";
            var tony = "<?=$this->view->tony?>";
            ...
            if( /Android/i.test(navigator.userAgent)) {
               window.lighthouse.dot(str, board_idx, category, tony);
               window.lighthouse.uppdate();
            }else if(/iPhone|iPad|iPod/i.test(navigator.userAgent)) {
                var array = [str, board_idx, category,tony];
                try {
                    window.webkit.messageHandlers.dot.postMessage(array);
                    window.webkit.messageHandlers.uppdate.postMessage();
                } catch (e) {
                }
            }
    	});
    ...
    <article class="view-area relative">
        ...
        <!--S:이미지,글-->
            <div class="content relative">
                <div class="txt-box" style="background-image:url('/public/uploads/mobile/image/<?=$this->view->image?>');">
                    <input type = 'hidden' id = 'iftony' value = '<?=$this->view->tony?>'>
                    <?if($this->view->color == 0) $color = 'textwhite'; else $color = 'textblack';
            			// $content = str_replace(PHP_EOL, ("<br />".PHP_EOL), $this->view->content);?>
                    <div class="txt ">
                        <textarea class="<?=$color?>" readonly onfocus="this.blur();"><?=$this->view->content?></textarea>
                    </div>
                    <div class="hashtag center <?=$color?>">
                        <?$tagarray = explode(',',$this->view->hashtag);
        ...
        <ul class="btm-box list-box">
            <li class="left">
                ...
                <button type="button" class = 'boardlike' data-board = '<?=$this->view->board_idx?>'><img src="<?=$img?>" alt="좋아요"><span><?=$this->favoritereplycon->allcount?></span></button> <!--[D] 좋아요 클릭시 아이콘 바뀜 /public/template/app/img/like_on.png -->
